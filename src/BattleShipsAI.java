...
public class BattleShipsAI implements Serializable {
    ...
    /**
     * Coords of last known strike. Would be a good idea to target next hits aroundf
     * this point.
     */
    ...
    public void putShips(AbstractShip ships[]) {
        ...
        for (AbstractShip s : ships) {
            do {
                ...
                x = rnd.nextInt(size);
                y = rnd.nextInt(size);
                int pick = rnd.nextInt(orientations.length);
                s.setOrientation(orientations[pick]);
    ...
    private boolean isUndiscovered(int x, int y) {
        return x >= 0 && x < size && y >= 0 && y < size && board.getHit(x, y) == Hit.NOT;
    }
